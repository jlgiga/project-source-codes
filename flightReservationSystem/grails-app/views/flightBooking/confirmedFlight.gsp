...
<html>
  ...
  <body onload="focusQueryInput();">
	...
	<div id="templatemo_middle_sub_flightList">
		...
		<p>As you can see, your PAYMENT STATUS is still stated as RESERVED. So the next thing your going to do is to pay for your flight reservation at the nearest flight ticketing outlet.</p>
	</div>
	<div id="templatemo_main">
		<g:form controller="flightBooking" action="print">
		  <br/><br/>
		  <p>Flight Details</p>
		  <table>
				<thead>
				...
				<tbody>
					<g:each in="${result2}" status="i" var="rs">
						<input type="hidden" name="flightBooking.id" value="${rs.flightBookingId}"/>
						<input type="hidden" name="flight.id" value="${rs.flightId}"/>
						<input type="hidden" name="flightBookingPayment.id" value="${rs.flightBookingPaymentId}"/>
		  ...
		  <table>
				<tbody>
					<g:each in="${result1}" status="i" var="resu">
						<input type="hidden" name="passenger.id" value="${resu.passengerId}"/>
						<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
